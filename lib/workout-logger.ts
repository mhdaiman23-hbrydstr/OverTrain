...
export class WorkoutLogger implements SetSyncProvider {
  ...
  /**
   * Set completion batching - queue and flush every 1-2 seconds
   * Prevents UI blocking when logging sets to Supabase
   */
  private static setCompletionQueue: Array<{
    workoutId: string
    exerciseId: string
    exerciseName: string
    setNumber: number
  ...
  private static queueSetCompletion(params: {
    ...
    // Add to queue
    this.setCompletionQueue.push(params)
    console.log(`[WorkoutLogger] Queued set completion, queue size: ${this.setCompletionQueue.length}`)
  ...
  /**
   * Flush all queued set completions to database
   * Batches multiple sets into single database writes
   */
  private static async flushSetCompletions(): Promise<void> {
    if (this.setCompletionQueue.length === 0) {
      this.flushTimer = null
      return
    }

    const queueSize = this.setCompletionQueue.length
    ...
    try {
      // Batch insert all set completions
      const { error } = await supabase
        .from("workout_sets")
        .upsert(
          itemsToFlush.map((item) => ({
            id: `${item.workoutId}-${item.exerciseId}-${item.setNumber}`,
            user_id: item.userId,
  ...
  static async getInProgressWorkout(week: number, day: number, userId?: string): Promise<WorkoutSession | null> {
    ...
    try {
      // Use async storage helpers (IndexedDB on mobile, localStorage fallback)
      ...
      if (instanceId && !match.programInstanceId) {
        ...
        workouts[matchIndex] = updated
        // Save updated workouts back
        await this.saveInProgressWorkoutsAsync(storageKeys.inProgress, workouts)
        return updated
      }
  ...
  static async saveCurrentWorkout(workout: WorkoutSession, userId?: string): Promise<void> {
    ...
    try {
      // RACE CONDITION FIX: Atomically read-modify-write using async storage
      await StorageLock.withLock(storageKeys.inProgress, async () => {
        // Use async storage helpers (IndexedDB on mobile, localStorage fallback)
        let workouts = await this.getInProgressWorkoutsAsync(storageKeys.inProgress)
        ...
        // Save using async storage (non-blocking, high-capacity)
        await this.saveInProgressWorkoutsAsync(storageKeys.inProgress, workouts)
        console.log("[WorkoutLogger.saveCurrentWorkout] Saved to IndexedDB/localStorage successfully")
      })
  ...
  static async logSetCompletion(
    ...
    // Queue for batched sync - returns immediately
    this.queueSetCompletion({
      workoutId,
      exerciseId,
      exerciseName,
      setNumber,
    ...
    // Backup to localStorage as well for safety
    try {
